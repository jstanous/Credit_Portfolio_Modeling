...
- **Trigger:** Runs on push to `main` in the private repo, scoped to changes in `/credit_portfolio` and `/models/credit_portfolio`.
...
- A private repo secret `PAT_Public_CreditPortfolioModeling` tied to a Personal Access Token (PAT) with content read/write permissions to the public repo.
